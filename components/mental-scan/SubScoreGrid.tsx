...
const METRICS = [
  {
    key: 'impulseControl'   as const,
    label: 'Impulse Control',
    desc: 'How quickly you exit — especially winners',
    icon: '⚡',
  },
  {
    key: 'diamondHands'     as const,
    label: 'Diamond Hands',
    desc: 'Patience on winning trades over time',
    icon: '💎',
  },
  {
    key: 'winRateQuality'   as const,
    label: 'Win Rate Quality',
    desc: 'Win rate weighted by profit factor',
    icon: '🎯',
  },
  {
    key: 'riskManagement'   as const,
    label: 'Risk Management',
    desc: 'Position sizing consistency & discipline',
    icon: '🛡️',
  },
  {
    key: 'emotionalControl' as const,
    label: 'Emotional Control',
    desc: 'Revenge trading & tilt streak detection',
    icon: '🧠',
  },
]
  ...
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
      {METRICS.map(m => {
        ...
        return (
          <div key={m.key} className="token-card group">
      ...
      <div className="token-card flex flex-col justify-center items-center text-center border-dashed">
        ...
        <p className="text-gray-500 text-xs leading-relaxed">
          Scores calculated from your actual on-chain trade history using FIFO position reconstruction.
          Based on hold times, P&L ratios, and behavioral pattern detection.
        </p>
      </div>
    </div>
  )
